...
// in simulator we redefine to format for CCR Developer console
...
{\
	NSString * exceptionName = [@"org.ccr." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.ccr";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	TiExceptionThrowWithNameAndReason(exceptionName,message);\
}\
...
enum {
	UICCRNativeItemNone = -1, 
	UICCRNativeItemSpinner = -2,
	UICCRNativeItemProgressBar = -3,
	
	UICCRNativeItemSlider = -4,
	UICCRNativeItemSwitch = -5,
	UICCRNativeItemMultiButton = -6,
	UICCRNativeItemSegmented = -7,
	
	UICCRNativeItemTextView = -8,
	UICCRNativeItemTextField = -9,
	UICCRNativeItemSearchBar = -10,
	
	UICCRNativeItemPicker = -11,
	UICCRNativeItemDatePicker = -12,
	
	UICCRNativeItemInfoLight = -13,
	UICCRNativeItemInfoDark = -14,
	
	UICCRNativeItemDisclosure = -15,
	
	UICCRNativeItemContactAdd = -16
};
...
/**
 *	TiThreadPerformOnMainThread should replace all CCR instances of
